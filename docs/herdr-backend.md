...
Firstmate requires Herdr protocol 14 or newer; broad backend verification covers versions 0.7.1, 0.7.3, 0.7.4, 0.7.5, and 0.8.0, while active required-CI evidence covers Herdr 0.8.2 protocol 20.
...
Projecting each task into its own workspace makes every task cleanup a workspace-emptying removal, which is the only removal shape Herdr's pre-0.8.0 focus defect touches, and the focus-safe removal plan below can only avoid it while the closing pane's shell can be proved idle with no children except the verified Herdr v0.8.2 restored-qterm topology: process-info names `zsh (qterm)` as the foreground shell and the only direct child is a childless `/bin/zsh --login`, with `comm` reported as `/bin/zsh` by BSD `ps` and `zsh` by Linux procps.
A persistent non-helper child of that shell - a `gitstatusd`, a `zsh-async` worker, or `direnv` - fails that proof permanently and forces the plain explicit close, which on those releases moves the active workspace for roughly a seventh of a second before the restore backstop pulls it back, once per task cleanup.
...
Projected cleanup therefore runs under the same session lock, captures the exact active tab, refuses to delete the active tab, and treats a workspace-emptying close as a focus-safe removal: it verifies the close would empty the workspace, repositions the doomed workspace behind the focused one through the verified `workspace.move` transport when needed, proves the pane holds one idle shell with no non-helper descendants or exactly the restored-qterm child, and ends that shell so Herdr removes the emptied workspace through its focus-preserving pane-death path.
...
Every proved or ambiguous workspace-removal cleanup therefore waits under the session lock for two consecutive exact-workspace-absence responses before it can complete the durable-record transaction, including a plain explicit-close fallback selected after the idle-shell proof refuses a non-helper child.
...
Forced secondmate cleanup recursively preflights every Herdr child endpoint and acquires every affected named-session lock before mutating any child, then retains each child's durable identity unless that exact pane returns structured not-found after its close; a child with a presentation journal also requires confirmed removal of the recorded presentation workspace before its records or owning home can be retired.
...
The process proof requires one recognized idle shell as both the shell process and the sole foreground process-group member, an operating-system process-table row for that shell, and a sleeping or idle shell state.
The only accepted child row is the Herdr v0.8.2 restored-qterm shape: the shell process itself is `zsh (qterm)`, and its sole direct child has `args` `/bin/zsh --login`, no descendants, and `comm` exactly `/bin/zsh` on BSD `ps` or `zsh` on Linux procps.
...
The matching journal is retired only after the close helper has positively confirmed exact pane removal and required presentation workspace removal; an unconfirmed close, an ambiguous topology whose workspace remains present or unknown, or a workspace-removal timeout retains the journal, while a confirmed removal may retire it even when focus restoration reported an error after the close.
...
- Crashes, lost responses, failed exact-pane cleanup, or human renames can leave quarantined spaces; session start removes only the exact home-local, uniquely journal-correlated idle-shell shape above, with only the verified restored-qterm child exception allowed.
...
The adapter does not promote that `pending` verdict from a rendered-footer transition on a `blocked` or unreadable native baseline; without a native idle baseline, that transition is not ownership proof for this Enter.
