...
The only accepted child rows are direct, childless, sleeping or idle helper children whose OS command is the same bare recognized shell shape observed in Herdr v0.8.2 qterm-restored panes.
...
- Crashes, lost responses, failed exact-pane cleanup, or human renames can leave quarantined spaces; session start removes only the exact home-local, uniquely journal-correlated idle-shell shape above, with only the verified qterm helper-shell exception allowed.
