...
The only accepted child row is the Herdr v0.8.2 restored-qterm shape: the shell process itself is `zsh (qterm)`, and its sole direct child has `comm` `/bin/zsh`, `args` `/bin/zsh --login`, and no descendants.
