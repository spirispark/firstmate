...
Cursor delivery confirmation is verified on tmux only.
On Herdr, Zellij, cmux, and Orca a Cursor steer can land, but `fm-send` may report delivery unconfirmed and exit non-zero because those adapters lack safe ownership proof for Cursor's rendered busy footer; [runtime backend verification](verification/runtime-backends.md#cursor-agent-cli) owns the evidence and transcript-state boundary.
