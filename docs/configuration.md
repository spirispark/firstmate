...
FM_WATCH_ARM_RETIRE_TIMEOUT_MS=1000   # milliseconds Pi/OpenCode wait for an unready successor arm to exit before abandoning retries; Pi bounds its pre-respawn stdio drain by the same value, so one Pi retirement can spend up to twice it
