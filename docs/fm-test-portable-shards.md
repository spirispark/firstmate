...
A runner executes one job at a time, so that holds while the labeled runners stay isolated from each other, including the container-isolated self-hosted runners `.github/workflows/ci.yml` selects.
