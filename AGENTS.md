...
Before changing the pipeline's engine order, run `bin/fm-pipeline-engine.sh` and decide from the quota evidence it reports; it reports by default and writes the order only with `--apply`, and it never restarts the shared no-mistakes daemon.
