#!/usr/bin/env bash
# Behavior tests for bin/fm-install-node.sh's architecture support
# contract. The script is the single owner of the CI's pinned Node.js
...
# the aarch64 branch; the historical GitHub-hosted runner path exercised
# x86_64. A future PR that drops one of the two arches silently regresses
# that consumer without a CI red flag, which is what these cases prevent.
#
...
# tests/node-install-helpers.sh, which only this suite drives today; if a
# second suite ever shims the same installer, hoist that helper file
# out so both share one shim set.
set -u

# shellcheck source=tests/node-install-helpers.sh
# shellcheck disable=SC1091
. "$(dirname "${BASH_SOURCE[0]}")/node-install-helpers.sh"

INSTALL="$ROOT/bin/fm-install-node.sh"
VERSION="$FM_NODE_CI_VERSION"
...
test_aarch64_host_installs_the_aarch64_asset() {
  ...
  tmp=$(fm_test_tmproot fm-node-aarch64)
  fakebin=$(fm_node_fakebin "$tmp" Linux aarch64)
  destination="$tmp/bin"

  out=$(fm_run_install "$fakebin" "$tmp" "$FM_NODE_CI_SHA256_AARCH64" "$destination") || rc=$?
...
test_arm64_alias_installs_the_aarch64_asset() {
  local tmp fakebin destination out rc=0
  tmp=$(fm_test_tmproot fm-node-arm64)
  fakebin=$(fm_node_fakebin "$tmp" Linux arm64)
  destination="$tmp/bin"
...
test_x86_64_host_installs_the_x86_64_asset() {
  local tmp fakebin destination out rc=0
  tmp=$(fm_test_tmproot fm-node-x86)
  fakebin=$(fm_node_fakebin "$tmp" Linux x86_64)
  destination="$tmp/bin"
...
test_cross_arch_digest_is_refused() {
  local tmp fakebin destination out rc=0
  tmp=$(fm_test_tmproot fm-node-cross-arch)
  fakebin=$(fm_node_fakebin "$tmp" Linux aarch64)
  destination="$tmp/bin"
...
test_unsupported_platform_is_refused_before_download() {
  local tmp fakebin destination out rc=0
  tmp=$(fm_test_tmproot fm-node-unsupported)
  fakebin=$(fm_node_fakebin "$tmp" Darwin arm64)
  destination="$tmp/bin"
...
test_wrong_binary_version_is_refused() {
  local tmp fakebin destination out rc=0
  tmp=$(fm_test_tmproot fm-node-wrong-version)
  fakebin=$(fm_node_fakebin "$tmp" Linux aarch64)
  destination="$tmp/bin"
