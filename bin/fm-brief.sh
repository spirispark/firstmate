...
\`resolved\` separately closes an escalated decision or blocker, and only a \`resolved [key=<slug>]: {how it was decided or unblocked}\` line carrying that decision's exact key closes it: a later \`done\` or \`working\` event never does, even when the answer is what started that work.
...
6. If a decision belongs to a human (product choices, destructive actions),
   ...
   A decision or blocker you opened stays open until \`resolved [key=<slug>]: {how it was decided or unblocked}\` carrying its exact key lands; a later \`done:\` or \`working:\` line never closes it, even when the answer is what started that work.
   Firstmate's reply normally writes that closing line at answer time; when a blocker or wait clears WITHOUT a firstmate reply, append \`resolved [key=<slug>]: {how it cleared}\` yourself as you resume.
