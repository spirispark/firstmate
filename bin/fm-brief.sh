...
When a decision you escalated is answered or a blocker clears and your domain resumes, append \`resolved [key=<slug>]: {how it was decided or unblocked}\` (using the same \`<slug>\` you opened it with) so it is durably closed instead of resurfacing behind later unrelated events.
...
6. If a decision belongs to a human (product choices, destructive actions),
   ...
   When firstmate replies or a blocker clears and you resume, append \`resolved [key=<slug>]: {how it was decided or unblocked}\` (using the same \`<slug>\` you opened it with) so the decision or blocker is durably closed and does not keep resurfacing.
